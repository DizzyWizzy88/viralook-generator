...
import { LogOut, Zap, LayoutGrid, Sparkles, Wand2, Globe, CreditCard, CheckCircle2 } from 'lucide-react';
      ...
      <AnimatePresence>
        {showSuccess && (
          <motion.div 
            ...
            className="fixed bottom-10 left-1/2 -translate-x-1/2 z-[100] bg-blue-600 px-8 py-4 rounded-2xl shadow-2xl flex items-center gap-4 border border-blue-400"
          >
            <CheckCircle2 className="text-white animate-bounce" />
            <div className="flex flex-col">
              <span className="font-black uppercase tracking-widest text-xs italic">Power Restored!</span>
              <span className="text-[10px] font-bold text-blue-100 uppercase tracking-tighter">Your credits have been added to the studio.</span>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
        ...
        <nav className="border-b border-white/5 bg-black/40 backdrop-blur-2xl sticky top-0 z-50">
          <div className="max-w-7xl mx-auto px-6 h-24 flex items-center justify-between">
            ...
            <div className="flex items-center gap-4">
              <button onClick={handleBilling} className="hidden md:flex items-center gap-2 px-4 py-2 rounded-full border border-white/10 hover:bg-white/5 transition-all text-[10px] font-black tracking-widest uppercase text-zinc-400">
        ...
        <main className="max-w-7xl mx-auto px-6 py-12 space-y-32">
          ...
          <section className="space-y-6">
            <div className="flex items-center gap-2 px-2">
              ...
              <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-zinc-600">Community Feed — Tap to Remix</h2>
            </div>
            <div className="flex gap-4 overflow-x-auto pb-4 no-scrollbar">
              {communityImages.map((img, i) => (
                <div key={i} onClick={() => handleRemix(img.prompt)} className="group relative flex-shrink-0 w-32 h-32 rounded-2xl overflow-hidden border border-white/5 cursor-pointer transition-all duration-500 hover:border-blue-500/50">
                  <img src={img.url} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" alt="Community" />
          ...
          <section className="max-w-4xl mx-auto text-center space-y-12">
            <div className="space-y-4">
              <div className="inline-flex items-center gap-2 px-4 py-1.5 rounded-full bg-blue-500/5 border border-blue-500/10 text-blue-500 mb-2">
                <Sparkles size={12} />
                <span className="text-[9px] font-black uppercase tracking-[0.3em]">Neural Engine v2 Active</span>
              </div>
              <h1 className="text-7xl md:text-8xl font-black tracking-tighter uppercase italic leading-[0.85]">
            ...
            <form onSubmit={generateImage} className={`relative max-w-2xl mx-auto group transition-all duration-700 ${loading ? 'scale-95 blur-[1px]' : ''}`}>
              ...
              />
              <button type="submit" disabled={loading} className="absolute right-4 top-4 bottom-4 px-10 bg-blue-700 hover:bg-blue-600 disabled:bg-zinc-800 rounded-[2rem] font-black text-xs tracking-widest uppercase flex items-center gap-3 transition-all">
